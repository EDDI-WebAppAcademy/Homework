<template>
  <div>
    ...
    <ul>
      <li>
        HP 교환 (경험치 10,000,000 당 50 up)
        <button v-bind:disabled="this.characterStatus.level < 99"  v-on:click="exchangeExpForHp()"> HP 50 교환 </button>
        <button class="btn_exchange">mP 50 교환 </button>
        <button disabled class="btn_exchange"> HP 50 교환 </button>
      </li>
    ...
    <button v-on:click="equipItem(e)" >아이템 장착!</button>
...
export default {
  ...
  methods: {
    exchangeExpForHp() {
      let remainingExp = this.characterStatus.currentLevelBar - this.characterStatus.totalLevelBar

      if (remainingExp >= 10000000) {
        this.characterStatus.hp += 50
        remainingExp -= 10000000
        this.characterStatus.currentLevelBar -= 10000000
        alert("교환할 수 있는 경험치 : " + remainingExp )
      } else if(remainingExp < 10000000) {
        this.characterStatus.hp += 0
  ...
  beforeUpdate() {
    ...
    let targetClass = document.querySelectorAll('.btn_exchange');
    if (this.characterStatus.level == 99) {
      targetClass.disabled = false
    } else {
      targetClass.disabled = true
    }


    let target = document.getElementById('exchangeBtn');
    if (this.characterStatus.level == 99) {
...
<!--         그 외 스탯도 1000만당 스탯 5 증가, hp, mp는 1000만당 50 증가로 작성해보도록 한다. &ndash;&gt;-->
<!--    <h3>경험치 교환 시스템</h3>-->
...
<!--    &lt;!&ndash; 현재 컨텐츠 측면에서 몬스터가 매번 같은것이 추가되서 재미 요소가 반감됨(그러므로 랜덤 요소를 넣어야함) &ndash;&gt;-->
<!--    (고정)몬스터 이름: <input v-model="name">-->
<!--    <button v-on:click="addFixedMonster">고정 몬스터 추가하기</button><br/>-->
