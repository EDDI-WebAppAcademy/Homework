<template>
  <div>
    ...
    <table border="1" v-if="expView">
      <tr>
        <th align="center" width="120">교환 리스트</th>
        <th align="center" width="40">교환</th>
      </tr>
      <tr v-for="(exchangeLists, idx) in expExchangeLists" :key="idx">
        <th align="center" width="120">{{ exchangeLists.name }}</th>
...
export default {
  ...
  methods: {
    ...mapActions([
      'requestExpTradeSystem'
    ]),
    async expTradeView() {
      await this.requestExpTradeSystem()
      this.expExchangeLists = this.$store.state.expTradeSystem
    }
