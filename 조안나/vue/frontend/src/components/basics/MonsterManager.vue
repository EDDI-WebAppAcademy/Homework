<template>
 <div>
   <button @click="addManyRandomMonster">랜덤 몬스터 100마리 추가하기(Local component)</button><br/>
...
export default {
  ...
  methods: {
    ...mapActions(['requestRandomMonsterAdd']),
    async addManyRandomMonster() {
      await this.requestRandomMonsterAdd()
      this.monsterLists = this.$store.state.randomMonstersList
    }
    ...
    //   let randomMonsterBookIdx = Math.floor(Math.random() * this.monsterBooks.length) //double
