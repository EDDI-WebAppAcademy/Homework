<template>
  <div>
    ...
    <button v-on:click="doManyExpExchange">교환</button>
    <table border="1">
      <tr>
...
export default {
  ...
  methods: {
    ...
    doManyExpExchange () {
      let tmpSum = 0
      // 여기에 스탯 적용 넣기
      if (this.characterStatus.currentLevelBar - tmpSum > 0) {
        for (let i = 0; i < this.exchangeListValue.length; i++) {
          console.log("expValue[0]: " + this.exchangeListValue[i] + ", ATK = " + ATK)
          if (this.exchangeListValue[i] === HP) {
            this.characterStatus.hp += HP_MP_INCREMENT
            tmpSum += 10000000
          } else if (this.exchangeListValue[i] === MP) {
            this.characterStatus.mp += HP_MP_INCREMENT
            tmpSum += 10000000
          } else if (this.exchangeListValue[i] === ATK) {
            console.log("Select ATK")
            this.characterStatus.atk += OTHER_STATS_INCREMENT
            tmpSum += 10000000
            this.characterStatus.atk = this.characterStatus.defaultAtk + this.characterStatus.itemAtk + this.exchangeStatus.atk
          } else if (this.exchangeListValue[i] === STR) {
            this.characterStatus.str += OTHER_STATS_INCREMENT
            tmpSum += 10000000
          } else if (this.exchangeListValue[i] === DEX) {
            this.characterStatus.dex += OTHER_STATS_INCREMENT
            tmpSum += 10000000
          } else if (this.exchangeListValue[i] === INT) {
            this.characterStatus.int += OTHER_STATS_INCREMENT
            tmpSum += 10000000
          } else if (this.exchangeListValue[i] === DEF) {
            this.characterStatus.def += OTHER_STATS_INCREMENT
            tmpSum += 10000000
          }
        }
        console.log("tmpSum: " + tmpSum)
        this.characterStatus.currentLevelBar -= tmpSum
      } else {
        alert("exp 부족!")
        return
      }
