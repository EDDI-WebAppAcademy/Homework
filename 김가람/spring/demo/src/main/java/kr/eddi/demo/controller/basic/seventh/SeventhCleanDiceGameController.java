...
public class SeventhCleanDiceGameController { // 7월 28일 수업
    ...
    // 특정 집합체를 다루는 가장 기본 형태는 배열이다.
    // 근데 배열의 단점은?
    // 1. 길이가 고정됨
    // 2. 한 가지 종류만 저장된다 ? (Object 타입 배열)
    // 3. 중간에 값을 끼워넣을 수 있을까?
    //      가능은 하지만 코드가 매우 복잡해짐
    //      기존 내용들을 전부 다 뒤로 빼고 해당 위치에 새로운 것을 배치해야함

    //      만약 배열 형태가 아니라 특정 정보를 끼워넣고 참조하는 위치만 변경할 수 있다면 어떨까?

    // * 배열 vs 연결리스트
    //            1  1  1  1  1  1  1  1
    // 배열의 형태 [0][1][2][3][4][5][6][7] <------- 구조 자체가 순차적임 (하드웨어적으로)
    //
    // 리시트의 형태 {1, 참조} ---> {1, 참조} ---> {1, 참조} ---> ...
    //
    ...
    public void createPlayerObj(List<Player> players, int playerObjNum) {
        for (int i = 0; i < playerObjNum; i++) {
            players.add(new Player()); // 생성자 기능 확인하기 !! 헷갈림!!
        }
    }
    ...
    public void gameStart() {
        for (int i = 0; i < playerNum; i++) {
            // 각 플레이어가 게임 실행하는 기능
            players.get(i).gameStart(players, i);
        }
    }
    ...
    public String checkWinner() {
        ...
        Collections.sort(players); // 점수를 오름차순으로 정렬한다.

       log.info("정렬 후: " + String.valueOf(players));

        int maxPlayerScore = acquirePlayerTotalScore(playerNum - 1);
        // 정렬 후 player1(get0) -> 낮은 점수, player2(get1) -> 높은 점수

        if (maxPlayerScore == DEATH_SCORE) { return "전원 탈락!"; }

        List<Integer> sameScoreIdxList = new ArrayList<>();

        for (int i = playerNum - 2; i >= 0; i--) { // 무승부 판정을 위한 코드
            // maxPlayerScore에는 높은 점수 할당. 그러므로 player1(get0)의 값만 알면 됨.
            int tmpScore = acquirePlayerTotalScore(i);

            if (maxPlayerScore != tmpScore) { break; } // 같지 않으면 멈춤.

            sameScoreIdxList.add(i); // 같으면 담음.
        }
