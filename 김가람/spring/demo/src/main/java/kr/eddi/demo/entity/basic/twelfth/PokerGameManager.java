...
import kr.eddi.demo.utility.basic.twelfth.BettingMoneyManager;
import lombok.Setter;

// 기능 위주(하향식 설계)로 가게 되면 머리만 아프고 결과는 제대로 안나올 가능성이 높음.
// 초보일때는 일단 기능을 먼저 구현해야 되서 하향식 설계를 위조로 하게 된다는 것.
// 결국 유저 스토리 기반의 백로그를 바라보고 컴포넌트를 볼 수 있어야 상향식 설계가 가능해짐.
@Setter
public class PokerGameManager {
    ...
    public void startGame() {
        // 3. 초기 베팅 머니를 부과한다.
        BettingMoneyManager.betMoney(DEFAULT_BETTING_MONEY * playerManager.getPlayerNum()); // 참여자 수 만큼의 베팅머니
    ...
    // 11. 최종 승자 판정
    public Player decisionWinner() {
        return new Player();
    }

    public Boolean isGameFinish() {
        if (roundManager.getCurrentRound() == 5) {
            return false;
        }
        return true;
    }
