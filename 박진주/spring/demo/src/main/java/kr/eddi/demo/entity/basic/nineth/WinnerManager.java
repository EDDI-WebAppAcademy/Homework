...
public class WinnerManager {
    private Map<Player, Integer> winnerScoreMap;                     // map 형식 winnerScoreMap 변수 선언 , Map < 키, 값>
    ...
    public List sortWinner(List<Player> players) {
        Score.calcPlayersScore(players);                                  // sortWinner 메소드 실행시 score가 계산됨

        for (int i = 0; i < players.size(); i++) {                         //winnerScoreMap에 플레이어 키값과 점수 저장
            winnerScoreMap.put(players.get(i), Score.getScore(i));
        }

        List<Map.Entry<Player, Integer>> winner = winnerScoreMap.entrySet().stream().sorted(Map.Entry.comparingByValue())      // 찾아보기
    ...
    public void doReward(List winner) {
        Map.Entry<Player, Integer> first = (Map.Entry<Player, Integer>) winner.get(winner.size() - 1);         // 정렬한 값 받는데 캐스팅했네
