...
import kr.eddi.demo.entity.basic.practice7_26.Player;
...
public class SeventhCleanDiceGameController {
    ...
    private List<Player> players; //Player형 리스트 players
    private int playerNum;//플레이어의 수

    @GetMapping("/dice-game")
    public String diceGameStarter() {

        createPlayer(); //플레이어를 만듭니다. 플레이어는 닉네임과 스코어를 갖습니다.
        gameStart();// 게임을 시작합니다. players에 담긴 플레이어가 순차적으로 주사위를 굴립니다.

        return checkWinner();//플레이어의 점수를 기준으로 정렬을 하고, 배열의 끝(1등)이 누구인지 리턴합니다.
    }
    ...
    public void createPlayer() {
        ...
        players = new ArrayList<>(); //players를 ArrayList로 초기화.
        this.playerNum = DEFAULT_PLAYER_NUM; //플레이어수를 2명으로 초기화
    ...
    public void createPlayerObj(List<Player> players, int playerObjNum) {
        for (int i = 0; i < playerObjNum; i++) {
            players.add(new Player()); // player수 만큼, players(어레이리스트)에 접근해서 Player객체 생성.
        }
    }
    ...
    //이 메소드는 players(어레이리스트)에 접근해서 첫번째 플레이어부터~마지막 플레이어까지
