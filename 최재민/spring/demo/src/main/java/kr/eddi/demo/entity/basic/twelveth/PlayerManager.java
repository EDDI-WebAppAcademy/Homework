...
public class PlayerManager {
    ...
    public void playPockerGame(int currentRound) {
        ...
        public int checkPlayersPriority() {
            ...
            public void publishingPlayerCard(int playerIdx) {
                List<Map<PockerCardShape, PockerCardCharacter>> existingPlayerPublishingDeckList;
                List<Map<PockerCardShape, PockerCardCharacter>> newPlayerPublishingDeckList = new ArrayList<>();

                int cardDividedCnt = playerList.get(playerIdx).getFirstCardDividedCnt();

                existingPlayerPublishingDeckList = playerList.get(playerIdx).acquireNewCards();
                newPlayerPublishingDeckList.add(playerList.get(playerIdx).publishingPlayersCard());

                int publisingDeckListSize = existingPlayerPublishingDeckList.size();

                if ( publisingDeckListSize == cardDividedCnt ){
                    playerPubishingDeck.put(playerList.get(playerIdx), existingPlayerPublishingDeckList);
                }else {
                    newPlayerPublishingDeckList = new ArrayList<>();
                    newPlayerPublishingDeckList.add(existingPlayerPublishingDeckList.get(publisingDeckListSize-1));
                    playerPubishingDeck.put(playerList.get(playerIdx) ,newPlayerPublishingDeckList);
                }
