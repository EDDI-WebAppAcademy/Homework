<template>
  <div>
    ...
    <label>
      <input type="checkbox" v-model="inventoryView" v-on:click="showInventory()">
      소지품 보기
    </label>
  ...
  methods:{
    ...
    async showInventory(){
      if(!this.inventoryView){
        ...
        this.myInventory = this.$store.state.addBuyItemsToInventory
      }
    },
    async equipItem () {
      let selectedItems = this.checkSelectedItemList()
      await this.requestEquipItem(selectedItems)
    },
    checkSelectedItemList (){
      let tmpList = []
      for (let i = 0; i < this.myInventoryValue.length; i++) {
        console.log(this.myInventoryValue[i])
